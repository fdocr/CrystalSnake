...
class Strategy::SafeEater < Strategy::Base
  def move(steps = 1)
    ...
    # enemy_moves = [] of String
    # permutations = 1
    # enemies.each_with_index do |snake, i|
    #   enemy_moves << @context.valid_moves(snake.head)[:moves]
    #   permutations = permutations * enemy_moves.last.count { true }
    # end
    ...
    #   enemy_moves[i].each_with_index do |dir, j|
    #     offset = enemy_moves[j]
    #     contexts.unsafe_fetch(i + (j * offset)).move(enemy.id, dir)
    #   end
    # end
